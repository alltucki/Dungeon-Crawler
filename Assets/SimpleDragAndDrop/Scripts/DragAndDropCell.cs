...
public class DragAndDropCell : MonoBehaviour, IDropHandler
{
    ...
    [Tooltip("Should this reference the player's equipment")]
    public bool is_equip_cell, is_inventory_cell;
    private bool had_item;
    ...
    public void update_inventory()
    {
        if (is_equip_cell)
        {
            if (had_item)
            {
                if (myDadItem == null)
                {
                    Debug.Log("Drag and drop cell unequipping slot " + equip_slot.ToString());
                    util_ref.p_manager.cur_player.GetComponent<player_script>().inv.unequip(equip_slot);
                    had_item = false;
                }
            }
            if (myDadItem != null)
            {
                Debug.Log("Drag and drop cell equipping slot " + equip_slot.ToString());
                util_ref.p_manager.cur_player.GetComponent<player_script>().inv.equip((equipment)myDadItem.attached_item, equip_slot);
                had_item = true;
            }
        }
        else if (is_inventory_cell)
        {
            if (had_item)
            {
                if (myDadItem == null)
                {
                    util_ref.p_manager.cur_player.GetComponent<player_script>().inv.drop(transform.GetSiblingIndex());
                    had_item = false;
                }
            }
            if (myDadItem != null)
            {
                util_ref.p_manager.cur_player.GetComponent<player_script>().inv.pickup((equipment)myDadItem.attached_item, transform.GetSiblingIndex());
                had_item = true;
            }
        }
